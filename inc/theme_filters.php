<?php
/*** Comment ***/
function mydecor_list_comments( $comment, $args, $depth ){
    switch ( $comment->comment_type ) :
            ...
            <div id="comment-<?php comment_ID(); ?>"  class="comment-wrapper">
                ...
                <div class="comment-detail">
                    ...
                    <?php if ( $comment->comment_approved == '0' ) : ?>
                        <em class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'mydecor' ); ?></em>
                    <?php endif; ?>
                    ...
                    <div class="comment-meta-bottom">
                        ...
                        <div class="comment-meta">
                            ...
                            <?php if( is_user_logged_in() ): ?>
                                <span class="edit button-text"><?php edit_comment_link( esc_html__( 'Edit', 'mydecor' ), '' ); ?></span>
                            <?php endif;?>
        ...
        case 'trackback' :
            ...
            <p><?php esc_html_e( 'Pingback:', 'mydecor' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( esc_html__( '(Edit)', 'mydecor' ), ' ' ); ?></p>
            <?php
            break;
    endswitch;
}

function mydecor_comment_form( $args = array(), $post_id = null ){
    ...
    $fields =  array(
      'author' => '<p class="author-row"><label>'.wp_kses( __('Your Name <span class="required">*</span>','mydecor'), $allowed_html ).'</label>' . '<input id="author" class="input-text" name="author" type="text" value="' .$comment_author. '" size="30"' . $aria_req . ' />' . '</p>'
    ,'email'  => '<p class="email-row"><label>'.wp_kses( __('Your Email <span class="required">*</span>','mydecor'), $allowed_html ).'</label><input id="email" class="input-text" name="email" type="text" value="' . $comment_author_email . '" size="30"' . $aria_req . '/>' . '</p>'
    );

    $required_text = sprintf( ' ' . wp_kses( __('Required fields are marked %s','mydecor'), $allowed_html ), '<span class="required">*</span>' );
    $defaults = array(
      'fields'               => apply_filters( 'comment_form_default_fields', $fields )
    ,'fields_before'		   => '<div class="info-wrapper">'
    ,'fields_after'		   => '</div>'
    ,'comment_field'        => '<div class="message-wrapper"><p><label>'.esc_html__('Your Comment', 'mydecor').'</label><textarea id="comment" name="comment" cols="45" rows="8" aria-required="true"></textarea></p></div>'
    ,'must_log_in'          => '<p class="must-log-in">' .  sprintf( __( 'You must be <a href="%s">logged in</a> to post a comment.', 'mydecor' ), wp_login_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>'
    ,'logged_in_as'         => '<p class="logged-in-as">' . sprintf( __( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>', 'mydecor' ), admin_url( 'profile.php' ), $user_identity, wp_logout_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>'
    ,'comment_notes_before' => ''
    ,'comment_notes_after'  => ''
    ,'id_form'              => 'commentform'
    ,'id_submit'            => 'submit'
    ,'title_reply'          => esc_html__( 'Leave a comment', 'mydecor' )
    ,'title_reply_to'       => esc_html__( 'Leave a Reply to %s', 'mydecor')
    ,'cancel_reply_link'    => esc_html__( 'Cancel reply', 'mydecor' )
    ,'label_submit'         => esc_html__( 'Submit', 'mydecor' )
    );
...
/* kses allowed html */
add_filter('wp_kses_allowed_html', 'mydecor_wp_kses_allowed_html', 10, 2);
function mydecor_wp_kses_allowed_html( $tags, $context ){
    switch( $context ){
        case 'mydecor_tgmpa':
            $tags = array(
...
/* Body classes filter */
add_filter('body_class', 'mydecor_body_classes_filter');
function mydecor_body_classes_filter( $classes ){
